...
public class WitherEnchant implements Listener {
    ...
    private PotionEffect wither = new PotionEffect(PotionEffectType.WITHER, 20, 0);
    ...
    public void onHit(EntityDamageByEntityEvent e) {
        if(e.getEntity() != null && e.getEntity() instanceof Player && e.getDamager() != null && e.getDamager() instanceof Player) {
            ...
            && ((Player) e.getDamager()).getInventory().getItemInMainHand().getItemMeta().getLore().contains(ChatColor.translateAlternateColorCodes('&', "&5Wither"))) {
                Random random = new Random();
                int selection = random.nextInt(10);
                if(selection < 1) {
                    ((Player) e.getEntity()).addPotionEffect(wither);
                    e.getEntity().sendMessage(ChatColor.translateAlternateColorCodes('&',
                            "&9&lENCHANTS &8» &7You've been withered by &b" + e.getDamager().getName() + "'s &5Wither&7!"));
                }
            }
        }
    }
